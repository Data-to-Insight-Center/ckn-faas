...
def aggregate_by_maxsize(df: pd.DataFrame, success_only: bool = True) -> pd.DataFrame:
    ...
    grouped = grouped.sort_values(["main_policy", "ModelSize", "IAR"]).reset_index(drop=True)
    return grouped
...
def plot(grouped: pd.DataFrame, out_png: str) -> None:
    ...
    for (pol, ms), sub in grouped.groupby(["main_policy", "ModelSize"]):
        sub = sub.sort_values("IAR")   # ascending IAR
    ...
    ax.set_xlabel("Inter-Arrival Rate (ms)")
    ax.set_ylabel("Average Latency (ms)")
    ax.set_title("Average Latency vs Inter-Arrival Rate by Model Set Size")
    ax.grid(True, linestyle="--", alpha=0.5)
    ax.legend(ncol=1, fontsize=9)

    # IMPORTANT: no invert_xaxis() here, so x-axis is ascending
    fig.tight_layout()
    fig.savefig(out_png, dpi=200)
    fig.show()
    print(f"[OK] Plot saved to: {out_png}")
